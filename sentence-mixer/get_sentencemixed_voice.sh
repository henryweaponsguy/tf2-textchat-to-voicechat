...
mix_sentences() {
    ...
    # Check if a dictionary is available
    if [ -e "$sound_dir/_dictionary.sed" ]; then
        # Replace synonyms and unavailable forms of words with existing words
        read -r -a words <<< "$(sed -Ef "$sound_dir/_dictionary.sed" <<< "${words[*]}")"
        ...
        for word in "${words[@]}"; do
            ...
            if [[ ${#matches[@]} -gt 0 ]]; then
                ...
                selected="$sound_dir/_placeholder.wav"
            fi

            echo "file '$selected'"
            done > "/tts/word_list.txt"
    else
    ...
    # echo "file '/tts/sounds/_period.wav'" >> word_list.txt

    if [ -s "/tts/word_list.txt" ]; then
        ffmpeg -hide_banner -loglevel error -f concat -safe 0 -i "/tts/word_list.txt" \
