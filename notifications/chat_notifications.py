...
# Word whitelist:
# Example: "your_username|help|afk"
whitelisted_words = ""
...
re_whitelisted_words = re.compile(rf"{whitelisted_words or '.*'}", re.IGNORECASE)


def play_sound(sound):
    subprocess.run(
        ["paplay", "--client-name=chat-notification", sound],
...
with open(console_log, "r") as log:
    ...
    while True:
        ...
        if not re_whitelisted_names.search(line):
            continue
        # Keep messages only with whitelisted words
        if not re_whitelisted_words.search(line):
            continue
        ...
        # Match files
        matched_files = list(sound_dir.glob("*.*"))
