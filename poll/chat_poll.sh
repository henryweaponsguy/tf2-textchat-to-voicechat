...
play_sound() {
    paplay --device=virtual_speaker --client-name=poll "$sound_dir/${1}.wav" >/dev/null 2>&1
}
...
speak_text() {
    ...
    paplay --device=virtual_speaker --client-name=poll-announcer "$audio_file" >/dev/null 2>&1
    rm -f "$audio_file"
}
...
start_poll() {
    ...
    yes_count=$(grep -c $'\tyes$' "$vote_file")
    no_count=$(grep -c $'\tno$' "$vote_file")
...
while IFS= read -r line; do
        ...
        if [ -e "$poll_open_state_file" ]; then
            ...
            if ! grep -Fq "$(printf '%s\t' "$username")" "$vote_file"; then
                if [[ "$command_input" =~ ^y[a-zA-Z0-9_-]* ]]; then
                    printf '%s\t%s\n' "$username" "yes" >> "$vote_file"
                    play_sound "yes" &
                elif [[ "$command_input" =~ ^n[a-zA-Z0-9_-]* ]]; then
                    printf '%s\t%s\n' "$username" "no" >> "$vote_file"
                    play_sound "no" &
                fi
            fi
        fi
    ...
    grep --line-buffered "^\(\*DEAD\*\|\*SPEC\*\)\?\((TEAM)\)\? \?.\+ :  !\(startpoll\|poll\) " |
