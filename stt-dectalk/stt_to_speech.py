...
if len(sys.argv) > 1:
    if sys.argv[1] == "-h":
        ...
        print(
            f"python3 {sys.argv[0]}                       # Use the default input device"
        )
        print(
            f"python3 {sys.argv[0]} <audio device id>     # Use a specific input device"
        )
...
model = Model(str(script_dir / "models" / "vosk-model-en-us-gigaspeech"))
# model = Model(str(script_dir / "models" / "vosk-model-small-en-us"))
...
def speak_text(text):
    ...
    try:
        subprocess.run(
            ["say", "-pre", "[:name PAUL]", "-e", "1", "-a", text, "-fo", audio_file]
        )
        ...
        announcer_process = subprocess.Popen(
            ["paplay", "--device=virtual_speaker", "--client-name=dectalk", audio_file],
...
with sd.RawInputStream(
    ...
    while True:
        ...
        if rec.AcceptWaveform(data):
            ...
            if (
                line == "stop"
                and announcer_process
                and announcer_process.poll() is None
            ):
                announcer_process.terminate()
                continue
