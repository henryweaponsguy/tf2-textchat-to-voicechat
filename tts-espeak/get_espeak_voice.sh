...
speak_text() {
    ...
    #espeak -v en+m5 -w "$audio_file" "$text"
    espeak -v "en+${voices[RANDOM % ${#voices[@]}]}" -w "$audio_file" "$text"

    (
        paplay --device=virtual_speaker --client-name=espeak "$audio_file" >/dev/null 2>&1
        rm -f "$audio_file"
    ) &
}
