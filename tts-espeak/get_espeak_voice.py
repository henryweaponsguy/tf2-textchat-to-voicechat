...
def speak_text(text):
    ...
    with tempfile.NamedTemporaryFile(
        prefix="espeak_voice-", suffix=".wav", delete=False
    ) as tmp:
        audio_file = tmp.name
    ...
    try:
        ...
        subprocess.run(
            ["espeak", "-v", f"en+{random.choice(voices)}", "-w", audio_file, text]
        )

        subprocess.run(
            ["paplay", "--device=virtual_speaker", "--client-name=espeak", audio_file],
